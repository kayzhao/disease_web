package com.csu.bio.api.umls.search;

import com.csu.bio.api.umls.RestTicketClient;
import com.csu.bio.api.umls.SearchResult;
...
public class SearchTerms {
	...
	// version is not a required argument - if left out you will search against
	// the latest UMLS publication
	...
	public void SearchTerm() throws Exception {
		...
		do {
			...
			Response response = given()
					...
					// uncomment to return CUIs that have at least one matching
					// term from the US Edition of SNOMED CT
					// .param("sabs", "SNOMEDCT_US")
					// uncomment to return SNOMED CT concepts rather than UMLS
					// CUIs.
					...
					.get("/rest/search/" + version);

			String output = response.getBody().asString();
			Configuration config = Configuration.builder()
					.mappingProvider(new JacksonMappingProvider()).build();
			results = JsonPath.using(config).parse(output)
					.read("$.result.results", SearchResult[].class);
