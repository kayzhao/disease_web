...
import com.csu.bio.api.umls.ConceptLite;
...
public class RetrieveCui {
	...
	public void retrieveCui() throws Exception {
		...
		ConceptLite conceptLite = JsonPath.using(config).parse(output)
				.read("$.result", ConceptLite.class);

		System.out.println(conceptLite.getUi() + ": " + conceptLite.getName());
		System.out.println("Semantic Type(s): "
				+ join(conceptLite.getSemanticTypes(), ","));
		...
		System.out.println("Highest Ranking Atom: "
				+ conceptLite.getDefaultPreferredAtom());
