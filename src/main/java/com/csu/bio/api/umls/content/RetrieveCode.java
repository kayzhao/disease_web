...
public class RetrieveCode {
	...
	public void RetrieveCodes() throws Exception {
		...
		Response response = given().log().all().request().with()
				.param("ticket", ticketClient.getST(tgt)).expect()
				.statusCode(200).when().get(path);
		// response.then().log().all();;
		String output = response.getBody().asString();
		Configuration config = Configuration.builder()
				.mappingProvider(new JacksonMappingProvider()).build();
		SourceAtomClusterLite code = JsonPath.using(config).parse(output)
				.read("$.result", SourceAtomClusterLite.class);

		System.out.println(source + " code: " + code.getUi() + ": "
				+ code.getName());
