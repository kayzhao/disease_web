package com.csu.bio.api.umls.content;

import com.csu.bio.api.umls.AtomLite;
import com.csu.bio.api.umls.RestTicketClient;
import com.csu.bio.api.umls.SourceAtomClusterLite;
...
public class WalkHierarchy {
	...
	// specifying version is not required - if you leave it out the script will
	// default to the latest UMLS publication.
	...
	public void WalkHierarchy() throws Exception {
		...
		version = System.getProperty("version") == null ? "current" : System
				.getProperty("version");
		// if you do not specify a source vocabulary, the script assumes you're
		// searching for CUI
		String path = "/rest/content/" + version + "/source/" + source + "/"
				+ id + "/" + operation;
		...
		int results = 0;
		int pageCount;
		// calls to descendants may produce several hundred or even several
		// thousand results.
		...
		do {
			...
			Response response = given()
					...
					.param("pageNumber", page).expect().statusCode(200).when()
					.get(path);
			...
			pageCount = JsonPath.using(config).parse(output)
					.read("$.pageCount");

			// the HL7 sources return Atom objects, rather than
			// SourceAtomClusters
			if (source.equals("HL7V3.0") || source.equals("HL7V2.5")) {

				atoms = JsonPath.using(config).parse(output)
						.read("$.result", AtomLite[].class);
				...
				sourceAtomClusters = JsonPath.using(config).parse(output)
						.read("$.result", SourceAtomClusterLite[].class);
				...
				for (SourceAtomClusterLite sourceAtomCluster : sourceAtomClusters) {
					...
					System.out.println("Number of Atoms: "
							+ sourceAtomCluster.getAtomCount());
					System.out.println("Concepts: "
							+ sourceAtomCluster.getConcepts());
					System.out.println("Ancestors: "
							+ sourceAtomCluster.getAncestors());
					System.out.println("Parents: "
							+ sourceAtomCluster.getParents());
					System.out.println("Children: "
							+ sourceAtomCluster.getChildren());
					System.out.println("Descendants: "
							+ sourceAtomCluster.getDescendants());
					System.out.println("Highest Ranking Atom: "
							+ sourceAtomCluster.getDefaultPreferredAtom());
					System.out.println("-------");
				}
