...
import com.csu.bio.common.api.umls.AtomLite;
import com.csu.bio.common.api.umls.RestTicketClient;
import com.csu.bio.common.api.umls.SourceAtomClusterLite;
	...
	public void WalkHierarchy() throws Exception {
		...
		do {
			...
			Response response = given()
					...
					.param("ticket", ticketClient.getST(tgt)).param("pageNumber", page).expect().statusCode(200).when()
					.get(path);
			...
			if (source.equals("HL7V3.0") || source.equals("HL7V2.5")) {
				...
				sourceAtomClusters = JsonPath.using(config).parse(output).read("$.result",
						SourceAtomClusterLite[].class);
